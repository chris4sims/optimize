...
#'
#' @param fcn The objective function to be minimized.  If its return value has a "gradient"
...
#' @param x0  Initial value of the parameter vector.
#' @param H0  Initial value for the inverse Hessian.  Must be positive definite,
#' if used.  (Not used if attr(fcn,"hessian") exists.)
#' @param grad  If this is a numerical vector and \code{attr(fcn,"gradient")} is
...
#' @param crit  Convergence criterion.  Iteration will cease when it proves impossible
#' to improve the function value by more than this.
#' @param nit   Maximum number of iterations.
#' @param ...   A list of optional length of additional parameters that get handed
...
#' @export 
csminwelNew <-
function(fcn,x0,H0,...,grad=NULL,crit=1e-7,nit,Verbose=TRUE,Long=FALSE) {
  ...
  while( !done ) {
    ...
    ##-------------------------
    itct <- itct+1
    itout <- csminit(fcn,x0=x, f0=f, g0=g,badg=badg, H0=H, Verbose=Verbose, ...)
    f1 <- itout$fhat
    x1 <- itout$xhat
    fc <- itout$fcount
      ...
      if( wall1 && dim(H)[1] > 1) {
        ...
        if( wall2 ){
          ...
          if( normdx < 1e-13 ) { # two trial x's too close, can't traverse
            ...
            dim(gcliff) <- c(nx,1)
            itout <- csminit(fcn,x0=x,f0=f, g0=gcliff, badg=FALSE,H0=diag(nx), Verbose=Verbose, ...)
            f3 <- itout$fhat
            x3 <- itout$xhat
            fc <- itout$fc
